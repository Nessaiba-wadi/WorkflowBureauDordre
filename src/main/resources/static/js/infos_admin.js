...
function chargerStatistiquesUtilisateurs() {
    // Récupération des données depuis l'API
    fetch('http://localhost:8082/utilisateurs/statistiques')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            console.log("Données utilisateurs reçues:", data); // Pour déboguer

            // Mise à jour des compteurs d'utilisateurs
            document.getElementById('totalBO').textContent = data.utilisateursBO + ' actifs';
            document.getElementById('totalComptables').textContent = data.utilisateursComptables + ' actifs';
            document.getElementById('totalTresorerie').textContent = data.utilisateursTresorerie + ' actifs';
        })
        .catch(error => {
            console.error('Erreur:', error);

            // En cas d'erreur, afficher un message
            document.getElementById('totalBO').textContent = 'Erreur de chargement';
            document.getElementById('totalComptables').textContent = 'Erreur de chargement';
            document.getElementById('totalTresorerie').textContent = 'Erreur de chargement';
        });
}

// Ajouter l'appel à cette fonction au chargement de la page
document.addEventListener('DOMContentLoaded', function() {
    console.log("Page chargée, chargement des statistiques...");
    chargerStatistiquesUtilisateurs(); // Ajouter cette ligne à votre gestionnaire d'événements existant
    chargerStatistiquesCommandes();
    chargerCommandesAvecPagination();
    ...
    setInterval(chargerStatistiquesUtilisateurs, 300000);
